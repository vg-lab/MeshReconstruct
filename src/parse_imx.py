...
def parse_bpSurfaceComponent(node, vrml_filename, tab):
    for child in node:
        if child.tag == 'bpComponentGroup':
            parse_bpComponentGroup(child, vrml_filename, tab)
        elif child.tag == 'bpSurfacesViewer':
            parse_bpSurfacesViewer(child, vrml_filename, tab)

def parse_bpSurfacesViewer(node, vrml_filename, tab):
    shape = 'Shape {'
...
def parse_bpComponentGroup(node, vrml_filename, tab):
    ...
    for child in node:
        if child.tag == 'bpComponentGroup':
            ...
            parse_bpSurfaceComponent(child, vrml_filename, tab)
    tab = tab[:-2]
    append_to_vrml(vrml_filename, tab + ']' )
    tab = tab[:-2]
    append_to_vrml(vrml_filename, tab + '}' )

def parse_bpSurfaceApplication(node, vrml_filename, tab):
    for child in node:
        if child.tag == 'bpComponentGroup':
            parse_bpComponentGroup(child, vrml_filename, tab)
        elif child.tag == 'bpSurfacesViewer':
...
def parse_root(node, vrml_filename):
    ...
    if bpSurfaceApplication is not None:
        ...
        parse_bpSurfaceApplication(bpSurfaceApplication, vrml_filename, tab)
        print('[DONE]')
    else:
